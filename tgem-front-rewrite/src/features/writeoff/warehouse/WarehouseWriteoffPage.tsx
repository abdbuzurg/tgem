...
export type WarehouseWriteoffType = "loss-warehouse" | "writeoff-warehouse"
import useScrollPaginated from "@shared/hooks/useScrollPaginated"

const TITLES: Record<WarehouseWriteoffType, string> = {
  "loss-warehouse": "Акт утери со склада",
  "writeoff-warehouse": "Акт списания со склада",
}

interface Props {
  writeOffType: WarehouseWriteoffType
}

export default function WarehouseWriteoffPage({ writeOffType }: Props) {
  ...
  return (
    <main>
      ...
      <table className="table-auto text-sm text-left mt-2 w-full border-box">
        ...
        <tbody>
          {tableDataQuery.isLoading &&
            <tr><td colSpan={4}><LoadingDots /></td></tr>
          }
          {tableDataQuery.isError &&
            <tr><td colSpan={4} className="text-red font-bold text-center">{tableDataQuery.error.message}</td></tr>
          }
      ...
      {showAddModal &&
        <WriteoffMutationModal mode="create" locationKind="warehouse" writeOffType={writeOffType} setShowModal={setShowAddModal} />
      }
      {showEditModal &&
