...
export default function InvoiceReturnTeamMutationModal(props: Props) {
  ...
  const [invoiceData, setInvoiceData] = useState<IInvoiceReturn>(
    isEdit
      ? {
          dateOfInvoice: new Date(props.invoiceReturnTeam.dateOfInvoice),
          deliveryCode: props.invoiceReturnTeam.deliveryCode,
          districtID: 0,
          id: props.invoiceReturnTeam.id,
          notes: "",
          projectID: 0,
          returnerID: 0,
          returnerType: "team",
  ...
  useEffect(() => {
    if (allDistrictsQuery.isSuccess && allDistrictsQuery.data) {
      ...
      if (isEdit) {
        const district = allDistrictsQuery.data.find(val => val.name == props.invoiceReturnTeam.districtName)!
  ...
  useEffect(() => {
    if (allTeamsQuery.isSuccess && allTeamsQuery.data) {
      ...
      if (isEdit) {
        const team = allTeamsQuery.data.find(val => val.teamNumber == props.invoiceReturnTeam.teamNumber && val.teamLeaderName == props.invoiceReturnTeam.teamLeaderName)!
        setSelectedTeam({
  ...
  useEffect(() => {
    if (allWorkersQuery.isSuccess && allWorkersQuery.data) {
      ...
      if (isEdit) {
        const worker = allWorkersQuery.data.find(val => val.name == props.invoiceReturnTeam.acceptorName)!
        setSelectedAcceptor({
          label: worker.name,
          value: worker.id,
        })
  ...
  const invoiceMaterialsForEditQuery = useQuery<IInvoiceReturnMaterials[], Error, IInvoiceReturnMaterials[]>({
    queryKey: ["invoice-return-materials-for-edit", isEdit ? props.invoiceReturnTeam.id : 0, invoiceData.returnerType, invoiceData.returnerID],
    queryFn: () => getInvoiceReturnMaterialsForEdit(isEdit ? props.invoiceReturnTeam.id : 0, invoiceData.returnerType, invoiceData.returnerID),
    ...
    <Modal setShowModal={props.setShowModal} bigModal>
      <div className="mb-2">
        <h3 className="text-2xl font-medium text-gray-800">
          {isEdit ? `Изменение накладной ${invoiceData.deliveryCode}` : "Добавление накладной возврат из бригад"}
        </h3>
      </div>
