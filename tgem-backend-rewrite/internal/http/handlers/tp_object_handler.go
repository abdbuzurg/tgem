...
type tpObjectHandler struct {
	tpObjectUsecase usecase.ITPObjectUsecase
}

func NewTPObjectHandler(tpObjectUsecase usecase.ITPObjectUsecase) ITPObjectHandler {
	return &tpObjectHandler{
		tpObjectUsecase: tpObjectUsecase,
	}
}

type ITPObjectHandler interface {
...
func (handler *tpObjectHandler) GetAll(c *gin.Context) {
	projectID := c.GetUint("projectID")

	data, err := handler.tpObjectUsecase.GetAllOnlyObjects(projectID)
	if err != nil {
...
func (handler *tpObjectHandler) GetPaginated(c *gin.Context) {
	...
	data, err := handler.tpObjectUsecase.GetPaginated(page, limit, filter)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}

	dataCount, err := handler.tpObjectUsecase.Count(filter)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *tpObjectHandler) Create(c *gin.Context) {
	...
	data, err := handler.tpObjectUsecase.Create(createData)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *tpObjectHandler) Update(c *gin.Context) {
	...
	data, err := handler.tpObjectUsecase.Update(updateData)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *tpObjectHandler) Delete(c *gin.Context) {
	idRaw := c.Param("id")
	...
	err = handler.tpObjectUsecase.Delete(uint(id), projectID)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *tpObjectHandler) GetTemplateFile(c *gin.Context) {
	templateFilePath := filepath.Join("./internal/templates/", "Шаблон для импорта ТП.xlsx")

	tmpFilePath, err := handler.tpObjectUsecase.TemplateFile(templateFilePath, c.GetUint("projectID"))
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *tpObjectHandler) Import(c *gin.Context) {
	...
	projectID := c.GetUint("projectID")
	err = handler.tpObjectUsecase.Import(projectID, filePath)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *tpObjectHandler) GetObjectNamesForSearch(c *gin.Context) {
	data, err := handler.tpObjectUsecase.GetObjectNamesForSearch(c.GetUint("projectID"))
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *tpObjectHandler) Export(c *gin.Context) {
	projectID := c.GetUint("projectID")

	exportFileName, err := handler.tpObjectUsecase.Export(projectID)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
