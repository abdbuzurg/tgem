...
type stvtObjectHandler struct {
	stvtObjectUsecase usecase.ISTVTObjectUsecase
}

func NewSTVTObjectHandler(stvtObjectUsecase usecase.ISTVTObjectUsecase) ISTVTObjectHandler {
	return &stvtObjectHandler{
		stvtObjectUsecase: stvtObjectUsecase,
	}
}

type ISTVTObjectHandler interface {
...
func (handler *stvtObjectHandler) GetPaginated(c *gin.Context) {
	...
	data, err := handler.stvtObjectUsecase.GetPaginated(page, limit, filter)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}

	dataCount, err := handler.stvtObjectUsecase.Count(filter)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *stvtObjectHandler) Create(c *gin.Context) {
	...
	data, err := handler.stvtObjectUsecase.Create(createData)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *stvtObjectHandler) Update(c *gin.Context) {
	...
	data, err := handler.stvtObjectUsecase.Update(updateData)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *stvtObjectHandler) Delete(c *gin.Context) {
	idRaw := c.Param("id")
	...
	err = handler.stvtObjectUsecase.Delete(uint(id), projectID)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *stvtObjectHandler) GetTemplateFile(c *gin.Context) {
	filepath := "./internal/templates/Шаблон для импорта СТВТ.xlsx"

	tmpFilePath, err := handler.stvtObjectUsecase.TemplateFile(filepath, c.GetUint("projectID"))
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *stvtObjectHandler) Import(c *gin.Context) {
	...
	projectID := c.GetUint("projectID")
	err = handler.stvtObjectUsecase.Import(projectID, filePath)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *stvtObjectHandler) GetObjectNamesForSearch(c *gin.Context) {
	data, err := handler.stvtObjectUsecase.GetObjectNamesForSearch(c.GetUint("projectID"))
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
...
func (handler *stvtObjectHandler) Export(c *gin.Context) {
	projectID := c.GetUint("projectID")

	exportFileName, err := handler.stvtObjectUsecase.Export(projectID)
	if err != nil {
		response.ResponseError(c, fmt.Sprintf("Внутренняя ошибка сервера: %v", err))
		return
	}
